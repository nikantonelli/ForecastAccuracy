Ext.define('ForcastAccuracy', {
    ...
    onSettingsUpdate: function() {
        //Do the same as if we restarted.....
        this.model = this.getSetting('piType') || 'Portfolioitem/Feature';
        this.start = new Date(this.getSetting('StartDate') || Ext.Date.now()-7776000000);
        this.end   = new Date(this.getSetting('EndDate') || Ext.Date.now());
...
//        this._fetchItems();
                ...
                load: function(store,records,status) {
                    if (!status) {
                        Rally.ui.notify.Notifier.showError({
                            message: 'Failed to fetch data from Rally',
                            timeout: 5000
                        });
                        me.hideMask();
                        return;
    ...
    _drawChart: function(id, title, data) {
        ...
        me.down('#chartBox').add( {
            ...
            itemId: id,
            chartColors: 
                ['rgba(27,158,119,0.5)','rgba(217,95,2,0.5)','rgba(117,112,179,0.5)','rgba(231,41,138)',
                 'rgba(102,166,30,0.5)','rgba(230,171,2,0.5)','rgba(166,118,29,0.5)','rgba(102,102,102,0.5)'],
            // chartColors: ['rgba(215,48,39, 0.7)','rgba(244,109,67, 0.7)','rgba(253,174,97, 0.7)','rgba(254,224,144, 0.7)',
            //                 'rgba(255,255,191,0.7)','rgba(224,243,248,0.7)','rgba(171,217,233, 0.7)','rgba(116,173,209,0.7)',
            //                 'rgba(69,117,180,0.7)'],
            chartConfig: {
                chart: {
                    type: 'scatter',
                ...
                xAxis: {
                    title: { text: 'PreliminaryEstimate Value of Portfolio Items'},
                    type: 'logarithmic'
                },
                yAxis: {
